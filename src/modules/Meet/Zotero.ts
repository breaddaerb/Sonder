...
import { supportsEmbeddings } from "../provider";
...
function fallbackDocScore(queryText: string, doc: Document) {
  ...
  tokens.forEach((token) => {
    ...
    score += Math.min(matches, 5) * Math.max(1, token.length / 2)
  })
  score += Math.min(doc.pageContent.length / 400, 8)
  return score
}

function fallbackRelatedDocs(queryText: string, docs: Document[]) {
  ...
  const ranked = docs
    .map((doc) => ({ doc, score: fallbackDocScore(queryText, doc) }))
    .sort((a, b) => b.score - a.score || b.doc.pageContent.length - a.doc.pageContent.length)
    .map((entry) => entry.doc)
  const selected: Document[] = []
  let usedChars = 0
  for (let doc of ranked) {
    if (selected.length >= Math.max(relatedNumber * 3, relatedNumber)) {
      break
    }
    ...
    selected.push(doc)
    usedChars += doc.pageContent.length
  }
  return selected.length > 0 ? selected : ranked.slice(0, relatedNumber)
}
...
export async function getRelatedText(queryText: string) {
  ...
  if (supportsEmbeddings()) {
    ...
    Meet.Global.popupWin?.createLine({ text: "Codex mode: using fallback context selection...", type: "default" })
