...
import { clearCodexLogin, finishCodexOAuthLogin, getCodexLoginReport, startCodexOAuthLogin } from "./Meet/CodexOAuth";
import { CODEX_MODELS, getCurrentModel, getProvider, hasCodexCredentials, setCurrentModel, setProvider, supportsEmbeddings } from "./provider";
...
// Temporarily disable markdown-it-mathjax3 here.
// Its bundled MathJax dependency uses eval("require"), which breaks in Zotero's plugin sandbox.
...
export default class Views {
  ...
  private buildContainer() {
    ...
    const that = this;
    let lastInputText = ""
    let inputListener = async function (event: KeyboardEvent) {
      // @ts-ignore
      ...
      if (event.key == "Enter") { 
        ...
        if (text.startsWith("#")) {
          ...
          if (key == "clear") {
            ...
            const secretKeyText = secretKey.length > 8 ? `${secretKey.slice(0, 3)}...${secretKey.slice(-4)}` : (secretKey || "(empty)")
            const reportLines = [
              `\`provider\` ${provider}`,
              `\`model\` ${getCurrentModel(provider)}`,
              `\`temperature\` ${Zotero.Prefs.get(`${config.addonRef}.temperature`)}`,
              `\`embeddings\` ${supportsEmbeddings(provider) ? "enabled" : "disabled"}`,
            ]
            if (provider == "openai-api") {
              ...
              reportLines.push(
                `\`oauth\` ${hasCodexCredentials() ? "configured" : "missing"}`,
                `\`oauthPending\` ${oauth.hasPendingLogin ? "yes" : "no"}`,
                `\`accountId\` ${oauth.accountId ? oauth.accountId.slice(0, 6) + "..." : "(missing)"}`,
                `\`expiresAt\` ${oauth.expires ? new Date(oauth.expires).toLocaleString() : "(missing)"}`,
              )
            }
            return that.setText(reportLines.join("\n"), true, false)
          } else if (key == "provider") {
            if (value?.length > 0) {
              if (["openai-api", "openai-codex"].indexOf(value) == -1) {
                return that.setText(`Invalid provider, ${value}. Use \`openai-api\` or \`openai-codex\`.`, true, false)
              }
            ...
            this.value = ""
            const provider = getProvider()
            return that.setText(provider == "openai-codex"
              ? "provider = openai-codex\nUse `/login` to authenticate with ChatGPT/Codex. Retrieval will use fallback context mode."
              : "provider = openai-api\nEmbeddings and retrieval are enabled when your OpenAI API key is configured."
            , true, false)
          } else if (key == "login") {
            if (getProvider() != "openai-codex") {
              return that.setText("Switch to `/provider openai-codex` before logging in.", true, false)
            }
            try {
              if (value?.length > 0) {
                const credentials = await finishCodexOAuthLogin(value)
                // @ts-ignore
                this.value = ""
                return that.setText(`Codex login succeeded.\n\`accountId\` ${credentials.accountId.slice(0, 6)}...\n\`model\` ${getCurrentModel("openai-codex")}`, true, false)
              }
              const auth = await startCodexOAuthLogin()
              Zotero.launchURL(auth.url)
              return that.setText([
                "Opened the ChatGPT login page in your browser.",
                "",
                "1. Finish login in the browser.",
                "2. The browser will likely show `ERR_CONNECTION_REFUSED` / `localhost refused to connect`.",
                "   That is expected in the current manual login flow.",
                "3. Copy the full URL from the browser address bar.",
                "4. Paste it back here as:",
                "",
                "```",
                "/login http://localhost:1455/auth/callback?code=...&state=...",
                "```",
                "",
                "You can also paste just the `code` value if needed.",
                `Redirect URI: \`${auth.redirectURI}\``,
              ].join("\n"), true, false)
            } catch (error: any) {
              return that.setText(String(error?.message || error), true, false)
            }
          } else if (key == "logout") {
            const hadCredentials = hasCodexCredentials() || getCodexLoginReport().hasPendingLogin
            clearCodexLogin()
            // @ts-ignore
            this.value = ""
            return that.setText(hadCredentials ? "Cleared Codex OAuth credentials." : "No Codex OAuth credentials were stored.", true, false)
          } else if (key == "model") {
            const provider = getProvider()
            if (value?.length > 0) {
              if (value == "default") {
                Zotero.Prefs.clear(`${config.addonRef}.${provider == "openai-codex" ? "codexModel" : "model"}`)
              } else {
                if (provider == "openai-codex" && CODEX_MODELS.indexOf(value as any) == -1) {
                  return that.setText(`Unknown Codex model, ${value}. Known models: ${CODEX_MODELS.join(", ")}`, true, false)
                }
                setCurrentModel(value, provider)
              }
            }
            // @ts-ignore
            this.value = ""
            return that.setText(`model (${provider}) = ${getCurrentModel(provider)}`, true, false)
          } else if (["secretKey", "api", "temperature", "deltaTime", "width", "tagsMore", "chatNumber", "relatedNumber", "codexModel"].indexOf(key) >= 0) {
            if (key == "codexModel") {
              if (value?.length > 0) {
                if (value == "default") {
                  Zotero.Prefs.clear(`${config.addonRef}.codexModel`)
                } else {
                  if (CODEX_MODELS.indexOf(value as any) == -1) {
                    return that.setText(`Unknown Codex model, ${value}. Known models: ${CODEX_MODELS.join(", ")}`, true, false)
                  }
                  Zotero.Prefs.set(`${config.addonRef}.codexModel`, value)
                }
              } else {
                value = Zotero.Prefs.get(`${config.addonRef}.codexModel`) as string
              }
              // @ts-ignore
              this.value = ""
              return that.setText(`codexModel = ${value || getCurrentModel("openai-codex")}`, true, false)
            }
            if (value?.length > 0) {
              if (value == "default") {
                Zotero.Prefs.clear(`${config.addonRef}.${key}`)
                value = Zotero.Prefs.get(`${config.addonRef}.${key}`) as string
        ...
        const commands = ["clear", "help", "report", "provider", "login", "logout", "model", "codexModel", "secretKey", "api", "temperature", "chatNumber", "relatedNumber" , "deltaTime", "tagsMore", "width"]
        that.createMenuNode(
          { x: rect.left, y: rect.top + rect.height, width: 220, height: 350 / 12 * commands.length  },
          commands.map(name => {
            return {
              name,
              listener: () => {
  ...
  private registerKey() {
    ...
    const isEditableTarget = (event: any) => {
      const target = event.originalTarget || event.explicitOriginalTarget || event.target || {}
      return Boolean(
        target.isContentEditable ||
        typeof target.value != "undefined" ||
        ["INPUT", "TEXTAREA"].indexOf(target.tagName) >= 0
      )
    }

    const isOpenShortcut = (event: any) => {
      const isSlashKey = event.code == "Slash" || event.key == "/" || event.key == "?"
      if (!isSlashKey || event.altKey) { return false }
      if (Zotero.isMac) {
    ...
    const isFallbackShortcut = (event: any) => {
      return Boolean(
        !event.metaKey &&
        !event.ctrlKey &&
        !event.altKey &&
    ...
    document.addEventListener(
      ...
      async (event: any) => {
        ...
        if ((isOpenShortcut(event) || isFallbackShortcut(event)) && !isEditableTarget(event)) {
          event.preventDefault();
          event.stopPropagation();
          await callback()
          return
