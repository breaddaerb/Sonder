...
\`/provider openai-api/openai-codex\` Switch between OpenAI API mode and ChatGPT/Codex OAuth mode.
\`/login\` Start ChatGPT/Codex OAuth login when provider is \`openai-codex\`.
\`/login <redirect-url>\` Finish ChatGPT/Codex OAuth login by pasting the redirect URL/code.
\`/logout\` Clear saved ChatGPT/Codex OAuth credentials.
...
\`/model gpt-4/gpt-3.5-turbo\` Set the active model for the current provider.
\`/codexModel gpt-5.2\` Set the saved Codex model directly.
...
You can exit me by pressing \`Esc\` above my head and wake me up by pressing \`Command + /\` on macOS, \`Ctrl + /\` on Windows/Linux, or \`Shift + ?\` as a fallback in the Zotero main window.
